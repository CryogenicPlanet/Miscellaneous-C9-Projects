  ...
  pythonProcess.stdout.on('data', async function(data) {
    ...
    let [user] = await con.query(`SELECT * FROM Users WHERE Name = ${data}`);
  ...
  app.get('/getLights', async function(req, res) {
    ...
    function newLight(Status, Red, Green, Blue) {
      this.status = Status;
      this.red = Red;
      this.green = Green;
      this.blue = Blue;
    }
    let lights = await con.query('SELECT * FROM Switchs');
    for (let light of lights) {
      sendlights.push(new newLight(light.Status, light.R, light.G, light.B));
    }
